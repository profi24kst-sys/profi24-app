...
(function(){
  ...
  async function next(anchor){const r=await loadOrder();if(!r)return;const role=user()?.role;const items=[];
    if(r.status==='NEW')items.push({label:'Назначить инженера и время',run:()=>{nativeMain();setTimeout(()=>document.querySelector('.sidecol')?.scrollIntoView({behavior:'smooth',block:'start'}),60)}});
    else if(r.status==='ASSIGNED'){if(role==='ENGINEER')items.push({label:'Принять заказ',run:async()=>{await api('/requests/'+r.id+'/accept',{method:'POST',body:'{}'});await window.Profi24OrderWorkspace?.refresh?.()}});else items.push({label:'Изменить назначение',run:()=>{nativeMain();setTimeout(()=>document.querySelector('.sidecol')?.scrollIntoView({behavior:'smooth',block:'start'}),60)}});if(!r.scheduled_at)items.push({label:'⚠ Не указано время выезда',disabled:true})}
