...
function retryAfterMs(response,attempt){const raw=response.headers.get('Retry-After');if(raw){const seconds=Number(raw);if(Number.isFinite(seconds))return Math.min(3000,Math.max(100,Math.ceil(seconds*1000)));const at=Date.parse(raw);if(Number.isFinite(at))return Math.min(3000,Math.max(100,at-Date.now()))}return Math.min(2000,500*(attempt+1))}
async function idempotentFetch(input,init,method){let response=await nativeFetch(input,init);if(!['GET','HEAD'].includes(method)||response.status!==429)return response;for(let attempt=0;attempt<2&&response.status===429;attempt++){const wait=retryAfterMs(response,attempt);console.warn('[PROFI24] Rate limited read; retrying',target(input),wait);await sleep(wait);response=await nativeFetch(input,init)}return response}
