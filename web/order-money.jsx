...
export function OrderPurchaseForm({requestId,onSaved}) {
  ...
  if(!open)return <button type="button" onClick={()=>setOpen(true)}>Купил запчасть — указать оплату</button>;
  return <form className="finOrderForm" onSubmit={f.submit(async(body,key)=>{await financeApi('/requests/'+requestId+'/part-purchases',{method:'POST',body:{...body,sale_price:body.sale_price||0},key});f.reset();await source.load();await onSaved();setOpen(false);})}>
