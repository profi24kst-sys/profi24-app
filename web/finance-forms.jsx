...
export function AccountForm({account,users,branches=[],close,done}) {
  const defaultBranch=account?.branch_id||((branches.length===1&&branches[0]?.id)||'');
  const f=useFinanceForm({name:account?.name||'',type:account?.type||'BANK',branch_id:defaultBranch,responsible_id:account?.responsible_id||'',comment:account?.comment||'',is_active:account?.is_active??true,initial_amount:'0',initial_reason:'',document_reference:''}),{form:b}=f;
  const save=async(body,key)=>{const data={...body,branch_id:body.branch_id?Number(body.branch_id):null,responsible_id:body.responsible_id?Number(body.responsible_id):null};if(account){delete data.initial_amount;delete data.initial_reason;delete data.document_reference;}await financeApi('/accounts'+(account?'/'+account.id:''),{method:account?'PATCH':'POST',body:data,key});done();};
  return <FinanceDialog title={account?'Редактирование счёта':'Новый денежный счёт'} close={()=>!f.busy&&close()}><form className="finForm" onSubmit={f.submit(save)}><fieldset disabled={f.busy}>
    ...
    <label>Филиал<select required value={b.branch_id||''} onChange={e=>{f.change('branch_id',e.target.value);f.change('responsible_id','')}}><option value="">Выберите филиал</option>{branches.map(x=><option key={x.id} value={x.id}>{x.name}{x.code?' · '+x.code:''}</option>)}</select></label>
    <label>Ответственный сотрудник<select required={['CARD','ADVANCE'].includes(b.type)} value={b.responsible_id} onChange={e=>f.change('responsible_id',e.target.value)}><option value="">Без персонального ответственного</option>{users.map(u=><option key={u.id} value={u.id} disabled={!u.active}>{u.name}{!u.active?' (неактивен)':''}</option>)}</select></label>
    <p className="finHint">Ответственный должен иметь доступ к выбранному филиалу. Для наличной кассы этот сотрудник будет отвечать за кассовую смену.</p>
    <label>Комментарий<textarea value={b.comment} onChange={e=>f.change('comment',e.target.value)} maxLength={1000}/></label>
    {account?<><label className="finCheck"><input type="checkbox" checked={b.is_active} onChange={e=>f.change('is_active',e.target.checked)}/>Счёт активен</label><p className="finHint">Остаток {financeMoney(account.balance)} не редактируется. Счёт с ненулевым остатком нельзя переносить в другой филиал — сначала оформите перевод.</p></>:<><label>Начальный остаток, ₸<input type="number" step="0.01" value={b.initial_amount} onChange={e=>f.change('initial_amount',e.target.value)}/></label>{Number(b.initial_amount)!==0&&<><label>Основание начального остатка<input required minLength={3} value={b.initial_reason} onChange={e=>f.change('initial_reason',e.target.value)}/></label><label>Номер акта / документа<input value={b.document_reference} onChange={e=>f.change('document_reference',e.target.value)}/></label><p className="finHint">Начальная сумма будет отдельной записью в журнале движений.</p></>}</>}
