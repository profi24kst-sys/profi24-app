import {financeApi,financeKey} from './finance-client.js';
// Order 360 payment modal: explicit account and retry-safe operation identifier.
(function(){
  ...
  async function open(detail={}){
    ...
    const id=Number(detail.id||window.Profi24O360State?.id||requestId());if(!id)return;
    try{
      const [r,accounts]=await Promise.all([api('/api/v1/requests/'+id),financeApi('/accounts')]);
      ...
      const select=sh.querySelector('[name="method"]');select.replaceChildren();
      const placeholder=document.createElement('option');placeholder.value='';placeholder.textContent='Выберите денежный счёт';select.appendChild(placeholder);
      accounts.filter(a=>a.is_active).forEach(a=>{const option=document.createElement('option');option.value=a.id;option.textContent=a.name;select.appendChild(option);});
      select.parentElement.firstChild.textContent='Зачислить на счёт';
      let operationId=financeKey();sh.addEventListener('input',()=>{operationId=financeKey();});sh.addEventListener('change',()=>{operationId=financeKey();});
      ...
      sh.querySelector('[data-amt="full"]').onclick=()=>{amount.value=debt;operationId=financeKey();calc()};const half=sh.querySelector('[data-amt="half"]');if(half)half.onclick=()=>{amount.value=Math.round(debt/2);operationId=financeKey();calc()};
      save.onclick=async()=>{try{err.textContent='';save.disabled=true;const a=Number(amount.value||0);if(a<=0)throw new Error('Введите сумму оплаты');if(a>debt)throw new Error(`Сумма оплаты не может превышать долг ${money(debt)}`);if(!select.value)throw new Error('Выберите денежный счёт');await api(`/api/v1/requests/${id}/payment`,{method:'POST',headers:{'Idempotency-Key':operationId},body:JSON.stringify({amount:a,account_id:Number(select.value),method:'ACCOUNT',reference:sh.querySelector('[name="reference"]').value.trim()||null})});close();window.dispatchEvent(new CustomEvent('profi24:request-updated',{detail:{id}}));window.dispatchEvent(new CustomEvent('profi24:open-order360',{detail:{id}}))}catch(e){err.textContent=e.message;save.disabled=false}};
