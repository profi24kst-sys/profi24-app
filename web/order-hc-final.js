...
(function(){
  ...
  function mount(){
    ...
    hero.innerHTML=`<div class="hcHeroLeft"><strong>Заказ ${rawNumber}</strong><span>${amount}</span><em class="hcHeroStatus ${status}">${labels[status]||pill?.textContent?.trim()||'—'}</em></div><div class="hcHeroActions"><button type="button" data-hc-final="print">▣ Печать</button><button type="button" data-hc-final="more">••• Еще</button></div>`;
