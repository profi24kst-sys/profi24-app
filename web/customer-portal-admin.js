...
async function createLink(button,id){
 ...
 try{
  const r=await fetch(`/approvals-api/api/v1/customer-portal/requests/${id}/link`,{method:'POST',headers:{Authorization:`Bearer ${auth()}`,'Content-Type':'application/json'},body:JSON.stringify({expires_days:30,send:true})});
...
function mount(){
 ...
 const root=document.querySelector('.o360[data-current-request-id]'),top=root?.querySelector('.o360Top');if(!root||!top||top.querySelector('[data-customer-portal-action]'))return;
 const id=Number(root.dataset.currentRequestId);if(!Number.isSafeInteger(id)||id<1)return;
 const button=document.createElement('button');button.type='button';button.dataset.customerPortalAction='true';button.title='Создать персональную ссылку на историю ремонтов';button.textContent='Кабинет клиента';button.addEventListener('click',()=>createLink(button,id));
