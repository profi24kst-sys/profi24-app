...
const HOLD_TYPES=new Set(['WAITING_CUSTOMER','WAITING_APPROVAL','WAITING_PART','EXTERNAL_SERVICE','WAITING_DELIVERY','OTHER']);
...
async function orderFor(req,{mutable=false,lock=false,client=pool}={}){
  if(req.order&&!lock)return req.order;
  return requireOrder(client,req.user,req.params.id,{mutable,lock});
}
async function sameBranchUser(c,userId,branchId,{engineer=false}={}){
  if(!userId)return null;
  const params=[userId,branchId],role=engineer?"AND u.role='ENGINEER'":'';
  return (await c.query(`SELECT u.id,u.name,u.role FROM users u JOIN user_branches ub ON ub.user_id=u.id WHERE u.id=$1 AND u.active=true AND ub.branch_id=$2 ${role} LIMIT 1`,params)).rows[0]||null;
}
async function lifecycleSnapshot(c,requestId){
  const [holds,visits,links]=await Promise.all([
  ...
  const activeHold=holds.rows.find(x=>!x.resumed_at)||null;
  return {active_hold:activeHold,holds:holds.rows,visits:visits.rows,links:links.rows};
}

app.get('/api/v1/requests/:id/lifecycle',{preHandler:auth},async(req,reply)=>{
  const order=await orderFor(req);if(!order)return fail(reply,'NOT_FOUND','Заказ не найден',404);
...
app.post('/api/v1/requests/:id/holds',{preHandler:office},async(req,reply)=>{
  ...
  const result=await tx(async c=>{
    ...
    if(responsibleId&&!await sameBranchUser(c,responsibleId,order.branch_id))throw Object.assign(new Error('Ответственный не относится к филиалу заказа'),{code:'RESPONSIBLE_BRANCH_MISMATCH',statusCode:422});
    const hold=(await c.query(`INSERT INTO request_holds(request_id,hold_type,reason,responsible_id,expected_until,pause_sla,previous_status,previous_sla_deadline,started_by)
      VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[order.id,type,reason,responsibleId,expected?.toISOString()||null,pauseSla,order.status,order.sla_deadline,req.user.id])).rows[0];
    if(pauseSla&&order.sla_deadline)await c.query('UPDATE requests SET sla_deadline=NULL,updated_at=now() WHERE id=$1',[order.id]);
    await c.query(`INSERT INTO request_history(request_id,user_id,action,details) VALUES($1,$2,'ORDER_HOLD_STARTED',$3)`,[order.id,req.user.id,{hold_id:hold.id,hold_type:type,reason,responsible_id:responsibleId,expected_until:expected?.toISOString()||null,pause_sla:pauseSla,previous_status:order.status,previous_sla_deadline:order.sla_deadline}]);
    return hold;
  });
...
app.post('/api/v1/requests/:id/holds/:holdId/resume',{preHandler:office},async(req,reply)=>{
  ...
  const result=await tx(async c=>{
    ...
    const updated=(await c.query('UPDATE request_holds SET resumed_by=$1,resumed_at=now(),resolution=$2 WHERE id=$3 RETURNING *',[req.user.id,resolution,hold.id])).rows[0];
    let deadline=order.sla_deadline;
    if(hold.pause_sla&&hold.previous_sla_deadline){
      const row=(await c.query('UPDATE requests SET sla_deadline=$1::timestamptz+(now()-$2::timestamptz),updated_at=now() WHERE id=$3 RETURNING sla_deadline',[hold.previous_sla_deadline,hold.started_at,order.id])).rows[0];
      deadline=row.sla_deadline;
    }
    await c.query(`INSERT INTO request_history(request_id,user_id,action,details) VALUES($1,$2,'ORDER_HOLD_RESUMED',$3)`,[order.id,req.user.id,{hold_id:hold.id,hold_type:hold.hold_type,resolution,duration_seconds:Math.max(0,Math.round((Date.now()-new Date(hold.started_at).getTime())/1000)),sla_deadline:deadline}]);
    return {...updated,new_sla_deadline:deadline};
  });
  return{data:result};
});
...
app.post('/api/v1/requests/:id/visits',{preHandler:office},async(req,reply)=>{
  ...
  const result=await tx(async c=>{
    const order=await requireOrder(c,req.user,req.params.id,{mutable:true,lock:true});
    const activeHold=(await c.query('SELECT id FROM request_holds WHERE request_id=$1 AND resumed_at IS NULL',[order.id])).rows[0];
    if(activeHold)throw Object.assign(new Error('Сначала завершите активное ожидание'),{code:'ORDER_ON_HOLD',statusCode:409});
...
app.post('/api/v1/requests/:id/visits/:visitId/outcome',{preHandler:auth},async(req,reply)=>{
  ...
  const result=await tx(async c=>{
    ...
    const updated=(await c.query('UPDATE request_visit_attempts SET outcome=$1,reason=$2,completed_by=$3,completed_at=now() WHERE id=$4 RETURNING *',[outcome,reason||null,req.user.id,visit.id])).rows[0];
    ...
    await c.query(`INSERT INTO request_history(request_id,user_id,action,details) VALUES($1,$2,'VISIT_OUTCOME_RECORDED',$3)`,[order.id,req.user.id,{visit_id:visit.id,attempt_no:visit.attempt_no,outcome,reason:reason||null}]);
    return updated;
  });
  return{data:result};
});
