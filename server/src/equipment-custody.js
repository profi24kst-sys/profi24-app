...
async function history(c,requestId){
 return (await c.query(`SELECT e.*,fu.name from_user_name,tu.name to_user_name,cb.name created_by_name FROM equipment_custody_events e LEFT JOIN users fu ON fu.id=e.from_user_id LEFT JOIN users tu ON tu.id=e.to_user_id LEFT JOIN users cb ON cb.id=e.created_by WHERE e.request_id=$1 ORDER BY e.id DESC`,[requestId])).rows;
}
function cleanAccessories(value){
 if(!Array.isArray(value))return [];
 return value.map(v=>text(v,120)).filter(Boolean).slice(0,50);
}

app.get('/api/v1/requests/:id/custody',{preHandler:auth},async(req)=>{
 const order=await requireOrder(pool,req.user,req.params.id);
 const events=await history(pool,order.id),current=events[0]||null;
 return{data:{order_id:order.id,request_number:order.number,request_status:order.status,current,events}};
});
...
app.post('/api/v1/requests/:id/custody/events',{preHandler:auth},async(req,reply)=>{
 ...
 const result=await tx(async c=>{
  ...
  const current=await latest(c,order.id,{lock:true});
  ...
  if(rule.engineerTarget){
   effectiveToUser=toUserId||positiveId(order.engineer_id);
   if(!effectiveToUser)throw Object.assign(new Error('Укажите инженера-получателя'),{code:'ENGINEER_REQUIRED',statusCode:422});
   if(Number(order.engineer_id)!==Number(effectiveToUser))throw Object.assign(new Error('Передача допускается только основному инженеру заказа'),{code:'PRIMARY_ENGINEER_REQUIRED',statusCode:409});
   if(!await sameBranchUser(c,effectiveToUser,order.branch_id,{engineer:true}))throw Object.assign(new Error('Инженер не относится к филиалу заказа'),{code:'ENGINEER_BRANCH_MISMATCH',statusCode:422});
  }
  if(rule.engineerReturn){
   const holder=positiveId(current?.to_user_id);
   if(!holder)throw Object.assign(new Error('В журнале не указан ответственный инженер'),{code:'CUSTODY_ENGINEER_MISSING',statusCode:409});
   if(req.user.role==='ENGINEER'&&Number(req.user.id)!==Number(holder))throw Object.assign(new Error('Инженер может вернуть только технику, которая числится за ним'),{code:'NOT_CUSTODIAN',statusCode:403});
  }
  const fromUser=positiveId(current?.to_user_id);
  const event=(await c.query(`INSERT INTO equipment_custody_events(request_id,event_type,from_holder,to_holder,from_user_id,to_user_id,location_text,condition_text,accessories,note,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11) RETURNING *`,[order.id,eventType,current?.to_holder||'CUSTOMER',rule.to,fromUser,effectiveToUser,location||null,condition||null,JSON.stringify(accessories),note||null,req.user.id])).rows[0];
  await c.query(`INSERT INTO request_history(request_id,user_id,action,details) VALUES($1,$2,'EQUIPMENT_CUSTODY_EVENT',$3)`,[order.id,req.user.id,{custody_event_id:event.id,event_type:eventType,from_holder:event.from_holder,to_holder:event.to_holder,from_user_id:fromUser,to_user_id:effectiveToUser,location_text:location||null,condition_text:condition||null,accessories}]);
  return event;
 });
 return reply.code(201).send({data:result});
 }catch(e){if(e?.statusCode)return fail(reply,e.code||'CUSTODY_ERROR',e.message,e.statusCode);throw e}
});
