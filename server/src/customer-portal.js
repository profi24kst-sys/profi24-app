...
import {publicApprovalToken} from './approval-token.js';
...
async function publicOrder(pool,row){
 ...
 if(await tableExists(pool,'customer_approvals')){
  const approval=(await pool.query(`SELECT request_id,version,token,token_nonce,status,expires_at FROM customer_approvals WHERE request_id=$1 ORDER BY version DESC LIMIT 1`,[row.id])).rows[0];
  const approvalToken=publicApprovalToken(approval);
  if(approval?.status==='PENDING'&&approvalToken&&new Date(approval.expires_at)>new Date())actions.approval_url=`/approve/${approvalToken}`;
 }
