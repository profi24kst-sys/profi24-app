...
BEGIN
  IF EXISTS(SELECT 1 FROM finance_bank_statements s WHERE s.id=OLD.statement_id AND s.status='RECONCILED') THEN
    RAISE EXCEPTION 'Строки сверенной банковской выписки неизменяемы' USING ERRCODE='P2401';
  END IF;
  RETURN CASE WHEN TG_OP='DELETE' THEN OLD ELSE NEW END;
END; $$ LANGUAGE plpgsql;
CREATE TRIGGER finance_bank_statement_line_guard BEFORE UPDATE OR DELETE ON finance_bank_statement_lines FOR EACH ROW EXECUTE FUNCTION finance_reconciled_line_immutable();
