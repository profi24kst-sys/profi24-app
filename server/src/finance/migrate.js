...
export async function migrateFinance(pool) {
  ...
  try {
    ...
    const applied = await client.query('SELECT version FROM finance_schema_migrations WHERE version=$1', ['001-auditable-accounts']);
    if (!applied.rows.length) {
      await client.query(await readFile(new URL('./001-auditable-accounts.sql', import.meta.url), 'utf8'));
      await client.query('INSERT INTO finance_schema_migrations(version) VALUES($1)', ['001-auditable-accounts']);
    }
