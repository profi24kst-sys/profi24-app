...
app.get('/api/v1/report',{preHandler:owner},async req=>{const start=monthStart(req.query?.month),end=monthEnd(start);
 ...
 const orders=(await q(`SELECT manager_id,count(*)::int jobs,COALESCE(sum(total),0)::numeric revenue,COALESCE(sum(total-direct_cost),0)::numeric gross_profit FROM requests WHERE status='CLOSED' AND closed_at>=$1 AND closed_at<$2 GROUP BY manager_id`,[start,end])).rows;
 const engOrders=(await q(`SELECT engineer_id,count(*)::int jobs,COALESCE(sum(total),0)::numeric revenue,COALESCE(sum(total-direct_cost),0)::numeric gross_profit FROM requests WHERE status='CLOSED' AND closed_at>=$1 AND closed_at<$2 AND engineer_id IS NOT NULL GROUP BY engineer_id`,[start,end])).rows;
 const works=(await q(`SELECT COALESCE(w.performed_by,r.engineer_id) user_id,COALESCE(sum(w.qty*w.unit_price),0)::numeric work_sales,COALESCE(sum(w.qty*w.direct_cost),0)::numeric work_cost FROM request_works w JOIN requests r ON r.id=w.request_id WHERE r.status='CLOSED' AND r.closed_at>=$1 AND r.closed_at<$2 GROUP BY COALESCE(w.performed_by,r.engineer_id)`,[start,end])).rows;
