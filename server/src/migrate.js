...
if(process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    ...
    console.log(`Applied ${coreStatements.length+branchStatements.length+lifecycleStatements.length+lifecycleV2Statements.length+lifecycleV3Statements.length+lifecycleV4Statements.length+payrollV2Statements.length+payrollKpiStatements.length} database migration statements`);
  } catch(error) {
