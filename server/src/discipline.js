...
app.get('/api/discipline/summary',{preHandler:management},async req=>{
  ...
  const resolvedHistory=(await q(`WITH cases AS (SELECT user_id,COALESCE(request_id::bigint,-id) case_id,MAX(weight)::int effective_weight FROM discipline_incidents WHERE resolved_at IS NOT NULL AND resolved_at>=now()-($1::int * interval '1 day') GROUP BY user_id,COALESCE(request_id::bigint,-id)) SELECT user_id,COUNT(*)::int cases,COALESCE(SUM(effective_weight),0)::int effective_weight FROM cases GROUP BY user_id`,[days])).rows;
  ...
  const map=a=>new Map(a.map(x=>[Number(x.user_id),x])),cm=map(current),pm=map(period),hm=map(resolvedHistory),vm=map(prev),am=map(active),acm=map(acceptance);
  const data=users.map(u=>{const c=cm.get(Number(u.id))||{},p=pm.get(Number(u.id))||{},h=hm.get(Number(u.id))||{},v=vm.get(Number(u.id))||{},a=am.get(Number(u.id))||{},ac=acm.get(Number(u.id))||{};const openCases=Number(c.cases||0),openSignals=Number(c.signals||0),openWeight=Number(c.effective_weight||0),periodCases=Number(p.cases||0),periodSignals=Number(p.signals||0),periodWeight=Number(p.effective_weight||0),historyWeight=Number(h.effective_weight||0),previousCases=Number(v.cases||0),currentPenalty=openWeight*8,historyPenalty=Math.min(20,historyWeight*2),rating=Math.max(0,100-currentPenalty-historyPenalty);return{id:u.id,name:u.name,role:u.role,active:Number(a.active||0),open_cases:openCases,open_incidents:openSignals,open_weight:openWeight,period_cases:periodCases,period_incidents:periodSignals,period_weight:periodWeight,resolved_period_cases:Number(h.cases||0),resolved_period_weight:historyWeight,previous_cases:previousCases,trend:periodCases-previousCases,rating,rating_current_penalty:currentPenalty,rating_history_penalty:historyPenalty,avg_accept_minutes:ac.avg_minutes==null?null:Number(ac.avg_minutes),accept_within_target_pct:ac.within_target==null?null:Number(ac.within_target)}});
  return{data:{days,generated_at:new Date().toISOString(),scoring:{dedupe:'max_weight_per_order',active_and_history_separated:true,current_weight_multiplier:8,history_weight_multiplier:2,history_penalty_cap:20},users:data}};
});
