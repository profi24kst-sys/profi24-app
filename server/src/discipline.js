...
function detect(r){
  ...
  const age=(now-updated)/36e5;
  const scheduled=r.scheduled_at?new Date(r.scheduled_at).getTime():0;
  const out=[];
...
async function scan(){
  const rows=(await q(`SELECT id,engineer_id,status,created_at,updated_at,scheduled_at,sla_deadline FROM requests WHERE status NOT IN ('CLOSED','CANCELLED')`)).rows;
  const seen=new Set();
  for(const r of rows){
    for(const type of detect(r)){
      seen.add(`${r.id}:${r.engineer_id}:${type}`);
      const weight=weights[type];
      await q(`INSERT INTO discipline_incidents(request_id,user_id,type,label,weight,last_seen_at,meta)
        VALUES($1,$2,$3,$4,$5,now(),$6)
        ON CONFLICT(request_id,user_id,type) WHERE resolved_at IS NULL
        DO UPDATE SET last_seen_at=now(),label=EXCLUDED.label,weight=EXCLUDED.weight,meta=EXCLUDED.meta`,
  ...
  for(const c of controls){
    ...
    await q(`INSERT INTO discipline_incidents(request_id,user_id,type,label,weight,last_seen_at,meta)
      VALUES($1,$2,'CONTROL',$3,$4,now(),$5)
      ON CONFLICT(request_id,user_id,type) WHERE resolved_at IS NULL
      DO UPDATE SET last_seen_at=now(),weight=EXCLUDED.weight,meta=EXCLUDED.meta`,
...
app.get('/api/discipline/summary',{preHandler:management},async req=>{
  ...
  const current=(await q(`WITH cases AS (
      SELECT user_id,COALESCE(request_id::bigint,-id) case_id,MAX(weight)::int effective_weight,COUNT(*)::int signals
      FROM discipline_incidents WHERE resolved_at IS NULL GROUP BY user_id,COALESCE(request_id::bigint,-id)
    )
    SELECT user_id,COUNT(*)::int cases,COALESCE(SUM(signals),0)::int signals,COALESCE(SUM(effective_weight),0)::int effective_weight
    FROM cases GROUP BY user_id`)).rows;

  const period=(await q(`WITH cases AS (
      SELECT user_id,COALESCE(request_id::bigint,-id) case_id,MAX(weight)::int effective_weight,COUNT(*)::int signals
      FROM discipline_incidents WHERE opened_at>=now()-($1::int * interval '1 day')
      GROUP BY user_id,COALESCE(request_id::bigint,-id)
    )
    SELECT user_id,COUNT(*)::int cases,COALESCE(SUM(signals),0)::int signals,COALESCE(SUM(effective_weight),0)::int effective_weight
    FROM cases GROUP BY user_id`,[days])).rows;

  const prev=(await q(`WITH cases AS (
      SELECT user_id,COALESCE(request_id::bigint,-id) case_id,MAX(weight)::int effective_weight
      FROM discipline_incidents
      WHERE opened_at<now()-($1::int * interval '1 day') AND opened_at>=now()-($1::int * 2 * interval '1 day')
      GROUP BY user_id,COALESCE(request_id::bigint,-id)
    ) SELECT user_id,COUNT(*)::int cases,COALESCE(SUM(effective_weight),0)::int effective_weight FROM cases GROUP BY user_id`,[days])).rows;
  ...
  let acceptance=[];
  try{
    acceptance=(await q(`WITH h AS (
      SELECT action,request_id,created_at,lead(created_at) OVER(PARTITION BY request_id ORDER BY created_at) next_at,lead(action) OVER(PARTITION BY request_id ORDER BY created_at) next_action
      FROM request_history WHERE created_at>=now()-($1::int * interval '1 day') AND action IN ('REQUEST_ASSIGNED','REQUEST_ACCEPTED')
    ),pairs AS (
      SELECT request_id,created_at assigned_at,next_at accepted_at FROM h WHERE action='REQUEST_ASSIGNED' AND next_action='REQUEST_ACCEPTED'
    ) SELECT r.engineer_id user_id,COUNT(*)::int accepted,
      round(avg(extract(epoch FROM(p.accepted_at-p.assigned_at))/60))::int avg_minutes,
      round(100.0*COUNT(*) FILTER(WHERE p.accepted_at-p.assigned_at<=interval '2 hours')/NULLIF(COUNT(*),0))::int within_target
      FROM pairs p JOIN requests r ON r.id=p.request_id WHERE r.engineer_id IS NOT NULL GROUP BY r.engineer_id`,[days])).rows;
  }catch(e){req.log.warn({err:e},'acceptance metrics unavailable')}

  const map=a=>new Map(a.map(x=>[Number(x.user_id),x]));
  const cm=map(current),pm=map(period),vm=map(prev),am=map(active),acm=map(acceptance);
  const data=users.map(u=>{
    const c=cm.get(Number(u.id))||{},p=pm.get(Number(u.id))||{},v=vm.get(Number(u.id))||{},a=am.get(Number(u.id))||{},ac=acm.get(Number(u.id))||{};
    const openCases=Number(c.cases||0),openSignals=Number(c.signals||0),openWeight=Number(c.effective_weight||0);
    const periodCases=Number(p.cases||0),periodSignals=Number(p.signals||0),periodWeight=Number(p.effective_weight||0),previousCases=Number(v.cases||0);
    const currentPenalty=openWeight*8;
    const historyPenalty=Math.min(20,periodWeight*2);
    const rating=Math.max(0,100-currentPenalty-historyPenalty);
    return{id:u.id,name:u.name,role:u.role,active:Number(a.active||0),open_cases:openCases,open_incidents:openSignals,open_weight:openWeight,period_cases:periodCases,period_incidents:periodSignals,period_weight:periodWeight,previous_cases:previousCases,trend:periodCases-previousCases,rating,rating_current_penalty:currentPenalty,rating_history_penalty:historyPenalty,avg_accept_minutes:ac.avg_minutes==null?null:Number(ac.avg_minutes),accept_within_target_pct:ac.within_target==null?null:Number(ac.within_target)};
  });
...
app.get('/api/discipline/incidents',{preHandler:management},async req=>{
  await scan();
  const days=Math.min(90,Math.max(1,Number(req.query?.days||30))),userId=Number(req.query?.user_id||0),p=[days],where=[`di.opened_at>=now()-($1::int * interval '1 day')`];
  if(userId){p.push(userId);where.push(`di.user_id=$${p.length}`)}
  const rows=(await q(`SELECT di.*,r.number,c.name customer_name,
    CASE WHEN di.weight>=3 THEN 'critical' WHEN di.weight=2 THEN 'warning' ELSE 'info' END severity
    FROM discipline_incidents di LEFT JOIN requests r ON r.id=di.request_id LEFT JOIN customers c ON c.id=r.customer_id
    WHERE ${where.join(' AND ')} ORDER BY di.opened_at DESC LIMIT 500`,p)).rows;
  return{data:rows};
});

setInterval(()=>scan().catch(e=>app.log.error(e)),300000);
scan().catch(e=>app.log.error(e));
const close=async()=>{await pool.end();process.exit(0)};
