export const coreStatements=[
  ...
  `CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY,name TEXT NOT NULL,email TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,role TEXT NOT NULL CHECK(role IN ('OWNER','SUPERVISOR','ACCOUNTANT','MANAGER','ENGINEER','TRAINEE')),active BOOLEAN DEFAULT TRUE,created_at TIMESTAMPTZ DEFAULT now())`,
  `ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check`,
  `ALTER TABLE users ADD CONSTRAINT users_role_check CHECK(role IN ('OWNER','SUPERVISOR','ACCOUNTANT','MANAGER','ENGINEER','TRAINEE')) NOT VALID`,
  ...
  `CREATE TABLE IF NOT EXISTS user_mentors(trainee_id INT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,mentor_id INT NOT NULL REFERENCES users(id),assigned_by INT REFERENCES users(id),assigned_at TIMESTAMPTZ NOT NULL DEFAULT now(),updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),CHECK(trainee_id<>mentor_id))`,
  `CREATE INDEX IF NOT EXISTS idx_user_mentors_mentor ON user_mentors(mentor_id)`,
  `CREATE TABLE IF NOT EXISTS request_participants(id SERIAL PRIMARY KEY,request_id INT NOT NULL REFERENCES requests(id) ON DELETE CASCADE,user_id INT NOT NULL REFERENCES users(id),participant_role TEXT NOT NULL CHECK(participant_role IN ('ENGINEER','TRAINEE')),mentor_id INT REFERENCES users(id),added_by INT REFERENCES users(id),added_at TIMESTAMPTZ NOT NULL DEFAULT now(),removed_at TIMESTAMPTZ)`,
  `CREATE INDEX IF NOT EXISTS idx_request_participants_request ON request_participants(request_id,removed_at)`,
  `CREATE INDEX IF NOT EXISTS idx_request_participants_user ON request_participants(user_id,removed_at)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uq_request_participants_active ON request_participants(request_id,user_id) WHERE removed_at IS NULL`,
