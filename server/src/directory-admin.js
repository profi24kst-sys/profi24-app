...
const owner=async(req,reply)=>{if(!await authenticate(req,reply,pool))return;if(req.user.role!=='OWNER')return fail(reply,'FORBIDDEN','Доступно только владельцу',403)};const directoryManager=async(req,reply)=>{if(!await authenticate(req,reply,pool))return;if(!roleAllowed(req.user.role,['OWNER','SUPERVISOR']))return fail(reply,'FORBIDDEN','Корзина доступна собственнику и управляющему',403)};
...
app.get('/api/v1/deleted',{preHandler:directoryManager},async()=>{const [customers,equipment]=await Promise.all([
