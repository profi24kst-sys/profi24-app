...
function nextFreeWindow(times,{date,now=new Date(),slotMinutes=90,startMin=9*60,endMin=18*60}={}){
 ...
 const blocks=(times||[]).map(toMin).filter(x=>Number.isFinite(x)).sort((a,b)=>a-b).map(x=>[x,x+slotMinutes]);
...
function rankForRequest(request,engineers,skillByEngineer){
 ...
 const ranked=engineers.filter(e=>Number(e.branch_id)===Number(request.branch_id)).map(e=>{
...
export async function buildEngineerCapacityAnalytics(db,{branchIds=null,branchId=null,date,now=new Date()}={}){
 ...
 const branchParams=[],branchScope=scope('r',branchIds,branchId,branchParams);if(branchScope.empty)return{generated_at:now.toISOString(),date,summary:{engineers:0,scheduled_jobs:0,overloaded:0,unassigned:0,overdue_sla:0,missed_visits:0},engineers:[],unassigned:[],methodology:{}};
 const branchWhere=branchScope.where.length?' AND '+branchScope.where.join(' AND '):'';
 ...
 if(engineerScopeIds){if(!engineerScopeIds.length)return{generated_at:now.toISOString(),date,summary:{engineers:0,scheduled_jobs:0,overloaded:0,unassigned:0,overdue_sla:0,missed_visits:0},engineers:[],unassigned:[],methodology:{}};engineerParams.push(engineerScopeIds);engineerWhere+=` AND (u.primary_branch_id=ANY($1::int[]) OR EXISTS(SELECT 1 FROM user_branches ux WHERE ux.user_id=u.id AND ux.branch_id=ANY($1::int[])))`}
 const engineerRows=(await db.query(`SELECT u.id,u.name,u.primary_branch_id,COALESCE((SELECT min(ux.branch_id) FROM user_branches ux WHERE ux.user_id=u.id),u.primary_branch_id) home_branch_id FROM users u WHERE ${engineerWhere} ORDER BY u.name`,engineerParams)).rows;
 if(!engineerRows.length)return{generated_at:now.toISOString(),date,summary:{engineers:0,scheduled_jobs:0,overloaded:0,unassigned:0,overdue_sla:0,missed_visits:0},engineers:[],unassigned:[],methodology:{}};
 const engineerIds=engineerRows.map(x=>Number(x.id));
 const params=[engineerIds,date];const extraScope=scope('r',branchIds,branchId,params);const extraWhere=extraScope.where.length?' AND '+extraScope.where.join(' AND '):'';
 const active=(await db.query(`SELECT r.id,r.number,r.engineer_id,r.branch_id,r.status,r.priority,r.sla_deadline,r.scheduled_at,r.created_at,e.category,TO_CHAR(r.scheduled_at AT TIME ZONE '${TZ}','HH24:MI') scheduled_local_time FROM requests r LEFT JOIN equipment e ON e.id=r.equipment_id WHERE r.deleted_at IS NULL AND r.engineer_id=ANY($1::int[]) AND r.status=ANY($3::text[])${extraWhere}`,[engineerIds,date,ACTIVE_STATUSES,...params.slice(2)])).rows;
 const dayRows=active.filter(x=>x.scheduled_at&&String(new Intl.DateTimeFormat('en-CA',{timeZone:TZ,year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(x.scheduled_at)))===date);
 const historyParams=[engineerIds,new Date(now.getTime()-180*86400000).toISOString()];const historyScope=scope('r',branchIds,branchId,historyParams);const historyWhere=historyScope.where.length?' AND '+historyScope.where.join(' AND '):'';
 ...
 const engineers=[];for(const e of engineerRows){const id=Number(e.id),mine=active.filter(x=>Number(x.engineer_id)===id),today=dayRows.filter(x=>Number(x.engineer_id)===id),overdue=mine.filter(x=>x.sla_deadline&&new Date(x.sla_deadline)<now),missed=mine.filter(x=>x.scheduled_at&&new Date(x.scheduled_at)<now&&['ASSIGNED','ACCEPTED','DIAGNOSTICS'].includes(x.status)),activeWeight=mine.reduce((a,x)=>a+(statusWeight[x.status]||.25),0),times=today.map(x=>x.scheduled_local_time).filter(Boolean),next=nextFreeWindow(times,{date,now});const branches=candidateBranches.get(id)||[];const branchForCard=branchId?Number(branchId):(branches[0]||Number(e.primary_branch_id)||0),scheduled=today.length,util=Math.min(150,scheduled/6*100),overloaded=scheduled>=6||activeWeight>=5||overdue.length>=2;engineers.push({id,name:e.name,branch_id:branchForCard,branch_ids:branches,scheduled_jobs:scheduled,active_orders:mine.length,active_weight:Number(activeWeight.toFixed(2)),overdue_sla:overdue.length,missed_visits:missed.length,utilization_pct:Number(util.toFixed(1)),next_free_time:next,overloaded,status:overloaded?'OVERLOADED':next?'AVAILABLE':'FULL'})}
 const unassignedParams=[];const unassignedScope=scope('r',branchIds,branchId,unassignedParams);const unassignedWhere=unassignedScope.where.length?' AND '+unassignedScope.where.join(' AND '):'';
