...
test('рейтинг поставщиков сравнивает одинаковые позиции, сроки, переплату и branch scope',async()=>{
 ...
 try{
  ...
  const item=(await query("INSERT INTO warehouse_items(branch_id,sku,name,oem_code,purchase_price) VALUES($1,'SUP-1','Компрессор SUP','OEM-SUP',1000) RETURNING id",[kst])).rows[0].id,remoteItem=(await query("INSERT INTO warehouse_items(branch_id,sku,name,oem_code,purchase_price) VALUES($1,'SUP-ALT','Чужая деталь','OEM-ALT',5000) RETURNING id",[alt])).rows[0].id;
  ...
  await query("INSERT INTO purchase_order_items(purchase_order_id,item_id,name,qty,unit_cost,received_qty) VALUES($1,$2,'Компрессор SUP',2,1250,0)",[po3,item]);
