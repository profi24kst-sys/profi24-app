...
test('physical equipment custody is branch-scoped, append-only and follows accountable handovers',async()=>{
 ...
 r=await call('POST',`/api/v1/requests/${request.id}/custody/events`,{event_type:'OFFICE_TO_CUSTOMER',recipient_name:'Иван Иванов',recipient_relation:'Клиент',recipient_confirmed:true},tokens.manager);assert.equal(r.status,409);assert.equal(r.error.code,'ORDER_NOT_CLOSED');
 await query("UPDATE requests SET status='CLOSED',closed_at=now() WHERE id=$1",[request.id]);
 r=await call('POST',`/api/v1/requests/${request.id}/custody/events`,{event_type:'OFFICE_TO_CUSTOMER'},tokens.manager);assert.equal(r.status,422);assert.equal(r.error.code,'RECIPIENT_REQUIRED');
 r=await call('POST',`/api/v1/requests/${request.id}/custody/events`,{event_type:'OFFICE_TO_CUSTOMER',recipient_name:'Иван Иванов',recipient_relation:'Клиент'},tokens.manager);assert.equal(r.status,422);assert.equal(r.error.code,'RECIPIENT_CONFIRMATION_REQUIRED');
 r=await call('POST',`/api/v1/requests/${request.id}/custody/events`,{event_type:'OFFICE_TO_CUSTOMER',recipient_name:'Иван Иванов',recipient_relation:'Клиент',recipient_confirmed:true,note:'Получатель проверил комплектность'},tokens.manager);assert.equal(r.status,201);assert.equal(r.data.to_holder,'CUSTOMER');assert.equal(r.data.recipient_name,'Иван Иванов');assert.equal(r.data.recipient_relation,'Клиент');assert.equal(r.data.recipient_confirmed,true);
 ...
 r=await call('GET',`/api/v1/requests/${request.id}/custody`,undefined,tokens.manager);assert.equal(r.status,200);assert.equal(r.data.current.to_holder,'CUSTOMER');assert.equal(r.data.current.recipient_name,'Иван Иванов');assert.equal(r.data.events.length,6);
 r=await call('GET',`/api/v1/requests/${request.id}/custody`,undefined,tokens.otherManager);assert.equal(r.status,403);
 const audit=(await query("SELECT action,details FROM request_history WHERE request_id=$1 AND action='EQUIPMENT_CUSTODY_EVENT' ORDER BY id",[request.id])).rows;assert.equal(audit.length,6);assert.equal(audit[0].details.event_type,'CUSTOMER_TO_OFFICE');assert.equal(audit.at(-1).details.event_type,'OFFICE_TO_CUSTOMER');assert.equal(audit.at(-1).details.recipient_name,'Иван Иванов');assert.equal(audit.at(-1).details.recipient_confirmed,true);
