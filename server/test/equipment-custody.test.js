...
test('physical equipment custody is branch-scoped, append-only and follows accountable handovers',async()=>{
 ...
 r=await call('POST',`/api/v1/requests/${request.id}/custody/events`,{event_type:'ENGINEER_TO_STORAGE',location_text:'Склад S-12'},tokens.otherEngineer);assert.equal(r.status,403);assert.equal(r.error.code,'NOT_CUSTODIAN');
