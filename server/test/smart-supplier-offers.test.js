...
test('актуальный прайс поставщика не перезаписывается старой ценой карточки склада',async()=>{
  ...
  const db={query:async sql=>{
    call++;const text=String(sql);
    if(text.includes('information_schema.tables'))return{rows:[{n:2}]};
    if(text.includes('supplier_catalog_links'))return{rows:[{item_id:7,supplier_id:11,supplier_name:'Live Supplier',unit_cost:1200,supplier_available_qty:5,lead_time_days:2,currency:'KZT'}]};
...
test('карточка склада остаётся fallback, когда связанного прайса вообще нет',async()=>{
  const db={query:async sql=>{const text=String(sql);if(text.includes('information_schema.tables'))return{rows:[{n:2}]};if(text.includes('supplier_catalog_links'))return{rows:[]};return{rows:[{item_id:8,supplier_id:12,supplier_name:'Fallback Supplier',unit_cost:900}]}}};
