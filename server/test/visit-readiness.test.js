...
test('visit readiness reminds once, escalates once, closes task and blocks reschedule route publication',async()=>{
 ...
 try{
  ...
  const request=(await pool.query("INSERT INTO requests(number,customer_id,equipment_id,engineer_id,branch_id,status,scheduled_at) VALUES('KST-READY',$1,$2,$3,$4,'ASSIGNED',$5) RETURNING id",[customer,equipment,engineer,branch,scheduled])).rows[0].id;
  ...
  const blocked=(await pool.query("INSERT INTO requests(number,customer_id,equipment_id,engineer_id,branch_id,status,scheduled_at) VALUES('KST-MOVE',$1,$2,$3,$4,'ASSIGNED',$5) RETURNING id",[customer,equipment,engineer,branch,scheduled])).rows[0].id;
  await pool.query(`INSERT INTO customer_visit_confirmations(request_id,customer_id,engineer_id,branch_id,version,token_nonce,token_hash,scheduled_at_snapshot,expires_at,status)
    VALUES($1,$2,$3,$4,1,'n2','h2',$5,$5+interval '12 hours','RESCHEDULE_REQUESTED')`,[blocked,customer,engineer,branch,scheduled]);
