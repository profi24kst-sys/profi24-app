...
function Wait-Http([string]$Url, [int]$DeadlineSeconds) {
...
Step 'Preparing local environment without overwriting existing secrets'
if (-not (Test-Path '.env')) {
  if (-not (Test-Path '.env.example')) { Fail '.env.example is missing.' }
  Copy-Item '.env.example' '.env'
  Write-Host 'Created .env from .env.example for local recovery only.' -ForegroundColor Yellow
}
...
$dbPassword = if ($envValues['POSTGRES_PASSWORD']) { $envValues['POSTGRES_PASSWORD'] } else { 'profi24-change-me' }
...
Compose up '-d' 'db'
if (-not (Wait-Http -Url 'http://127.0.0.1:1' -DeadlineSeconds 1)) { }
